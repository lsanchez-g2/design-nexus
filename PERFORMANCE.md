...
v3.3.0 replaces fixed 10-page batches with **adaptive 5-20 page batches** based on file density tier.

**Key Enhancements**:
1. **Lazy Density Detection**: Zero-overhead component counting during Pass 1
2. **Adaptive Batch Sizing**: Dynamic batch sizes (5-20 pages) based on detected tier
3. **Smart Progress Indicators**: Milestone-based updates with ETA calculation
4. **EXTREME Tier Handling**: User prompt for files >2000 component sets

### Density Tier Classification

| Tier | Criteria | Batch Size | Example System | Components/Page | Total Sets |
|---|---|---|---|---|---|
| EXTREME | >40 avg OR >2000 total | User prompt | Polaris | 55 | 3,500 |
| HIGH | 10-40 avg | 5 pages | Dense libraries | 15-30 | 500-2000 |
| MEDIUM | 3-10 avg | 10 pages | Carbon | 3.1 | 170 |
| LOW | 1-3 avg | 15 pages | Flowbite Pro | 1.2 | 250 |
| VERY_LOW | <1 avg | 20 pages | Preline UI | 0.3 | 30 |

### Performance Targets

| File Type | v3.2.1 Baseline | v3.3.0 Target | Expected Improvement |
|---|---|---|---|
| VERY_LOW (Preline-like) | 65s | ≤50s | 20-30% |
| LOW (Flowbite-like) | 130s | ≤120s | 10-15% |
| MEDIUM (Carbon-like) | 83s | ≤85s | 0-5% (no regression) |
| EXTREME (Polaris-like) | Manual sampling | Auto-detect prompt | Automated workflow |

**Validation Status**: Results pending — will update this section after running tests on Preline, Flowbite, Carbon, Polaris.

### Overhead Analysis

**Density detection overhead**: ~0.1s per page for `findAll()` type checks.

- 200-page file: ~20s total overhead
- Spread across Pass 1 (30-60s duration)
- **Net overhead**: <5% of Pass 1 duration (negligible)

### Dynamic Timeout Fallback

**Threshold**: If batch duration exceeds 40s (67% of MCP ~60s limit), batch size is halved automatically.

**Retry logic**:
1. Batch duration >40s → reduce batch size by 50%
2. Minimum batch size: 5 pages
3. Error if 5-page batch still fails (file too dense, recommend strategic sampling)

**Expected impact**: 0% timeout failures maintained (same as v3.2.1).
...
**v3.3.0 Adaptive Rate**:
- VERY_LOW density: ~2.5 pages/sec (0.4s/page, 2× faster than v3.2.1)
- LOW density: ~2.0 pages/sec (0.5s/page, 1.3× faster than v3.2.1)
- MEDIUM density: ~1.67 pages/sec (0.6s/page, same as v3.2.1)
- HIGH density: ~1.25 pages/sec (0.8s/page, conservative)

*(Projected — awaiting validation)*
